...
# 그룹은 콘솔 수동 생성 (CLAUDE.md 소유권 경계). ID는 display_name으로 조회한다.
data "databricks_group" "workspace_admins" {
  provider     = databricks.account
  display_name = "${local.name_prefix}-WS-ADMINS"
}

data "databricks_group" "catalog_admins" {
  provider     = databricks.account
  display_name = "${local.name_prefix}-CATALOG-ADMINS"
}

data "databricks_group" "workspace_users" {
  provider     = databricks.account
  display_name = "${local.name_prefix}-WS-USERS"
}

resource "databricks_mws_permission_assignment" "workspace_admins" {
  provider     = databricks.account
  workspace_id = azurerm_databricks_workspace.this.workspace_id
  principal_id = data.databricks_group.workspace_admins.id
  permissions  = ["ADMIN"]

  depends_on = [databricks_metastore_assignment.this]
}

# USER 바인딩: RBAC assume 대상 role 그룹도 workspace에 붙어야 role switcher에 노출된다
resource "databricks_mws_permission_assignment" "catalog_admins" {
  provider     = databricks.account
  workspace_id = azurerm_databricks_workspace.this.workspace_id
  principal_id = data.databricks_group.catalog_admins.id
  permissions  = ["USER"]
...
resource "databricks_mws_permission_assignment" "workspace_users" {
  provider     = databricks.account
  workspace_id = azurerm_databricks_workspace.this.workspace_id
  principal_id = data.databricks_group.workspace_users.id
  permissions  = ["USER"]
